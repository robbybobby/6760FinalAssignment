...
from rmu.utils_WMDP import load_model, get_params, forward_with_cache, get_data
    ...
    for epoch in range(1):
        ...
        with tqdm.tqdm(total=num_batches) as pbar:
            for idx in range(num_batches):
                ...
                if batch_idx >= len(forget_data_list[topic_idx]) or batch_idx >= len(retain_data_list[topic_idx]):
                    print(f"Skipping batch {batch_idx} for topic {topic_idx} (missing data)")
                    continue
                ...
                unlearn_inputs = {k: v.to(updated_model.device) for k, v in unlearn_inputs.items()}

                retain_inputs = tokenizer(
                    retain_batch, return_tensors="pt", padding=True, truncation=True, max_length=512
                )
                retain_inputs = {k: v.to(updated_model.device) for k, v in retain_inputs.items()}
                ...
                if args.verbose:
                    ...
                    print(f"unlearn_cosine_sim = {unlearn_cosine.item():.4f}")
                    print(f"retain_cosine_sim  = {retain_cosine.item():.4f}")
                    print(f"Topic {topic_idx} updated_forget_activations.norm = {torch.mean(updated_forget_activations.norm(dim=-1).mean(dim=1), dim=0).item():.4f}")
                    print(f"Topic {topic_idx} frozen_forget_activations.norm  = {torch.mean(frozen_forget_activations.norm(dim=-1).mean(dim=1), dim=0).item():.4f}")
                    print(f"Topic {topic_idx} updated_retain_activations.norm = {torch.mean(updated_retain_activations.norm(dim=-1).mean(dim=1), dim=0).item():.4f}")
                    print(f"Topic {topic_idx} frozen_retain_activations.norm  = {torch.mean(frozen_retain_activations.norm(dim=-1).mean(dim=1), dim=0).item():.4f}")
